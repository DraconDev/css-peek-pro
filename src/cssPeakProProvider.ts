...
export class CSSPeakProProvider implements vscode.HoverProvider {
  ...
  constructor(cssParser: CSSParser) {
    ...
    console.log("CSS Peek Pro: CSSPeakProProvider constructed");
  }
  ...
  provideHover(
    ...
    console.log("=== CSS Peek Pro: HOVER TRIGGERED ===");
    ...
    if (!enableHover) {
      console.log("CSS Peek Pro: Hover is disabled");
      return null;
    }
    ...
    if (!range) {
      console.log("CSS Peek Pro: No word found at position");
      return null;
    }

    const word = document.getText(range);
    console.log(`CSS Peek Pro: Found word: "${word}"`);
    ...
    if (!word || word.length === 0) {
      console.log("CSS Peek Pro: Empty word");
      return null;
    }

    // Very permissive CSS detection - accept anything that looks like text
    console.log(
      "CSS Peek Pro: ACCEPTING as potential CSS selector (testing mode)"
    );

    console.log(`CSS Peek Pro: Getting CSS rules for "${word}"`);
    ...
    console.log(`CSS Peek Pro: Found ${cssRules.length} CSS rules`);

    if (cssRules.length === 0) {
      console.log("CSS Peek Pro: No CSS rules found, returning null");
      return null;
    }

    console.log("CSS Peek Pro: Creating hover content...");
    const hoverContent = this.createHoverContent(cssRules, word);
    console.log("CSS Peek Pro: Hover content created");

    console.log("=== CSS Peek Pro: RETURNING HOVER ===");
  ...
  private createHoverContent(
    ...
    rulesByFile.forEach((fileRules, fileName) => {
      content.appendText(`______${fileName}______\n`);
